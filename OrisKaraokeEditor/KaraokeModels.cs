...
public sealed class KaraokeSong
{
    ...
    public static KaraokeSong FromPlainText(string title, string artist, string audio, string plainText)
    {
        var song = new KaraokeSong
        {
            Title = title.Trim(),
            Artist = artist.Trim(),
        ...
        foreach (string rawLine in normalized.Split('\n'))
        {
            ...
            int wordIndex = 0;
            foreach (string word in Regex.Split(lineText, @"\s+").Where(x => !string.IsNullOrWhiteSpace(x)))
            {
    ...
    public IEnumerable<KaraokeWordRef> AllWords()
    {
        for (int li = 0; li < Lines.Count; li++)
        {
            for (int wi = 0; wi < Lines[li].Words.Count; wi++)
                yield return new KaraokeWordRef(li, wi, Lines[li].Words[wi]);
        }
    }

    public KaraokeWordRef? WordAtFlatIndex(int index)
    {
        int i = 0;
    ...
    public void RecalculateLineTimes()
    {
        for (int i = 0; i < Lines.Count; i++)
        {
            ...
            {
                var timed = line.Words.Where(w => w.TimeMs > 0).ToList();
                if (timed.Count > 0)
                {
                    line.StartMs = timed.Min(w => w.TimeMs);
...
public sealed record KaraokeWordRef(int LineIndex, int WordIndex, KaraokeWord Word);
